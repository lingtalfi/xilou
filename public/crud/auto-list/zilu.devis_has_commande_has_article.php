...
d.commande_has_article_id,
c.unit as commande_has_article_unit
';
...
inner join zilu.commande_has_article c on c.id=d.commande_has_article_id
inner join zilu.devis de on de.id=d.devis_id
";


$table = CrudModule::getDataTable("zilu.devis_has_commande_has_article", $query, $fields, ['devis_id', 'commande_has_article_id']);
...
$table->columnLabels= [
    ...
    "commande_has_article_unit" => "commande has article",
];
...
$table->setTransformer('commande_has_article_unit', function ($v, array $item) {
    return '<a href="' . CrudHelper::getUpdateFormUrl('zilu.commande_has_article', $item['commande_has_article_id']) . '">' . $v . '</a>';
});
